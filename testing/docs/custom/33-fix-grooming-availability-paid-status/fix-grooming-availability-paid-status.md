...
- **`20260804092_m03_get_staff_availability_lunch_break.sql`** (one day later, adding the lunch-break check) redefined the *same* function again, but branched off the pre-083 body - so its Check 2 reverted to `status in ('Pending', 'In Progress', 'Completed', 'Paid')`, silently undoing 083's fix and reintroducing a reference to a status value that no longer exists in the enum.
