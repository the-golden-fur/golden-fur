-- Sprint 1 gap closure: get_staff_availability() (...014) predates the
-- status column added on staff_unavailability_blocks (...019, Jul 11, 2026
-- request/approval redesign) and was never updated to match it - it still
-- counts pending and denied rows as blocking availability. Per spec, only
-- status = 'approved' rows should. This mirrors the filter the read-side
-- reference implementation already applies correctly
-- (staffAvailability.service.ts's getStaffAvailability()).
--
-- Only the unavailability-overlap check (the final condition) changes; the
-- operating-hours check and the booking-conflict placeholder are unchanged
-- from ...014.
