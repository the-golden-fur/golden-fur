-- Fixes a merge collision between two same-day-adjacent migrations that both
-- redefined get_staff_availability() from the same 20260728062 base without
-- knowing about each other's change:
--   - 20260803083 dropped 'Paid' from the booking_status enum entirely
--     (payment tracking moved to the independent payment_stage column) and
--     redefined this function's Check 2 to match.
--   - 20260804092 (one day later) redefined the SAME function to add the
--     lunch-break check, but branched off the pre-083 body - so it
--     reintroduced Check 2's now-invalid 'Paid' literal, and the resulting
--     function has thrown `invalid input value for enum booking_status:
--     "Paid"` on every call ever since (surfaced via GET
--     /bookings/availability, e.g. picking Grooming in the booking flow).
--
-- CREATE OR REPLACE against the identical signature (same rationale as prior
-- redefinitions' own header notes) - no caller changes. Body is 20260804092
-- verbatim except Check 2 drops 'Paid' (matching 20260803083) and its
-- comment, which still described the older 'Confirmed' retirement, is
-- corrected to describe this one.
...
begin
  ...
  where sp.branch_id = p_branch_id
    ...
    -- Check 2 (fixed here): no overlapping booking in any status that still
    -- holds a real slot - Pending/In Progress/Completed. 'Paid' no longer
    -- exists as a status value (20260803083 - see payment_stage instead);
    -- 20260804092 had accidentally reintroduced it by branching off a
    -- pre-083 copy of this function.
    and not exists (
      select 1
