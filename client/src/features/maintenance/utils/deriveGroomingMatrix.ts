 ...
 * (#81; custom change: configurable pricing rules) - needed here too so the
 * service form's preview can compute a live matrix from a not-yet-saved
 * base_price, with no round trip to the server. There is no shared module
 * between the client and server builds, so keep this formula in sync with
 * the server copy if it ever changes.
 */
