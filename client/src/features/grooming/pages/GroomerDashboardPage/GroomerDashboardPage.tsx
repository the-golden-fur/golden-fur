...
export function GroomerDashboardPage() {
  ...
  useEffect(() => {
    ...
    void listGroomingQueue(token).then(handleQueueResult);
  ...
  return (
    <main className={styles.page}>
      ...
      {isLoading ? (
        <p className={styles.copy}>Loading today's appointments...</p>
      ) : loadError ? (
        <p className={styles.errorBanner} role="alert">
          {loadError}
        </p>
      ) : enriched.length === 0 ? (
        <p className={styles.copy}>No grooming appointments today.</p>
      ) : (
