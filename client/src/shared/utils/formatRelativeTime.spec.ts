...
describe('formatRelativeTime', () => {
  it('returns "today" for a timestamp within the last 24 hours', () => {
    expect(formatRelativeTime('2026-08-02T01:00:00.000Z', NOW)).toBe('today');
  });

  it('returns "yesterday" for a timestamp 1-2 days back', () => {
    expect(formatRelativeTime('2026-08-01T01:00:00.000Z', NOW)).toBe(
      'yesterday'
    );
  });
  ...
  it('returns "N years ago" past a year back', () => {
    expect(formatRelativeTime('2024-01-01T12:00:00.000Z', NOW)).toBe(
      '2 years ago'
    );
  });
});
