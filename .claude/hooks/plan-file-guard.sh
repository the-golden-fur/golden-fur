...
deny "No vault session plan.md found or recently touched under golden-fur-vault/Projects/golden-fur/sessions/. Per standing instruction, the final plan must be written there BEFORE exiting Plan Mode: create golden-fur-vault/Projects/golden-fur/sessions/${next_nn}-<slug>/plan.md (near-beginner style, per .agent/skills/session-documentation.md's template - same shape as the plan file you're about to submit here, but re-derive it, do NOT literally copy it). NEVER include any real secret or credential (API keys, tokens, passwords, .env values) in this file, even if the ~/.claude/plans working file has them - reference where the value lives instead (e.g. 'see temp.txt / the value already shared in chat'), per the skill's secrets carve-out. Then retry ExitPlanMode."
